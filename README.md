...
将整个目录放入 `.claude/skills/aigcpass/`：
...
```bash
pip3 install --break-system-packages rich pyyaml questionary
...
1. 运行 `script/setup.py` 创建 job 并配置 API key
...
以下命令 Agent 不会替你执行（它们需要终端交互）：

```bash
# 设置向导（Step 1）
python3 script/setup.py --jobid mypaper
...
每 job 独立配置 `jobs/{jobid}/api.yaml`。运行 `setup.py` 时，向导会带你交互式选择服务商、模型，并自动填写 endpoint 和参数。
...
如果你需要增加自己的降重文本审查规则（例如禁止特定词汇、检查句式结构等），编辑 `doc/check_optimization.md`。
...
如果你不想用 Agent，可以通过 `aigcpass.py` 统一入口完成所有操作（注意，不要移动这里aigcpass.py的路径，请直接使用其绝对路径以保证正确性）：
...
全部可用命令见 `python3 aigcpass.py --help`，详细参数说明见 `doc/scripts.md`。
