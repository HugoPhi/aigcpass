...
```
jobs/{jobid}/
  api.yaml                      # DeepSeek API 配置（每 job 独立）
  main.tex                      # LaTeX 源文件
  report/
    *.html                       # AIGC 检测报告
    AIGC片段优化.txt              # 降重后放入
...
每 job 独立配置 `jobs/{jobid}/api.yaml`，模板位于 `template/api.yaml.example`。
...
如果你不想用 Agent，可以通过 `aigcpass.py` 统一入口完成所有操作：
