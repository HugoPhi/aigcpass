...
向导会依次询问：
1. **选择服务商** — 从列表中选择（DeepSeek / Kimi / Qwen / GLM / OpenAI / Claude / Gemini）
2. **选择模型** — 从该服务商的推荐模型中选择，或手动输入
3. **输入 API key** — 密码输入，不会回显
4. **确认 endpoint** — 默认已填好，通常不需要改
5. **调整生成参数** — temperature（默认 0.3）、max_tokens（默认 8192）

完成后配置自动写入 `jobs/{jobid}/api.yaml`。
...
如果向导无法满足需求，可手动编辑 `jobs/{jobid}/api.yaml`：
...
**Q: 是否支持同时配置多个厂商？**
A: 每个 job 的 `api.yaml` 只能指定一个活跃厂商。如需切换，重新运行 `setup.py` 或手动编辑 `api.yaml`。
...
```bash
python3 -u script/stage2_api.py --jobid mypaper --dry-run
```
